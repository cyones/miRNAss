...
* features: is a data frame with the features extracted from hairpyn sequences, one sequence per row and one numeric feature per column. The hairpin sequences
can be extracted from a raw genome using HExtractor
(<https://sourceforge.net/projects/sourcesinc/files/hextractor/>), and the
features can be calculated using miRNAfe (<https://sourceforge.net/projects/sourcesinc/files/mirnafe/> or <http://fich.unl.edu.ar/sinc/blog/web-demo/mirnafe-full/>). The whole pipeline consist on extract the hairpins from a raw genome in fasta format. The output, also in fasta format is the input to miRNAfe, which as result gives a comma separate 
file that can be easily load into R.
